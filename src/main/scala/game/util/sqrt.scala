...
class InverseSqrt(errorMargin: Double = 0.1) extends Module {
  ...
  val xReg = Reg(SInt(24.W))
  val yReg = Reg(SInt(24.W))
  val readyReg = RegInit(false.B)

  when(io.wrEn) {
    xReg := io.input
    val xU = io.input.asUInt
    val leading = PriorityEncoder(Reverse(xU(23, 12))) // count leading zeros
    val shiftAmt = leading
    val normalized = (xU << shiftAmt)(23, 0).asSInt // shift to [1.0, 2.0)

    val lutIndex = (normalized.asUInt >> (24 - log2Ceil(lutSize))).asUInt
