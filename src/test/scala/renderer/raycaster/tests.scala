...
class RaycasterSpec extends AnyFunSpec with ChiselSim with Matchers {
  type Test = (Vec2D, Double, Int)

  describe("Raycaster") {
    it("calculates correct position for general angles") {
      ...
      testsBySteps.foreach { case (steps, testGroup) =>
        simulate(new Raycaster(steps)) { dut =>
          ...
          testGroup.foreach { case (pos, angle, _) =>
            ...
            withClue(
              f"Test: [${pos.x}%.1f,${pos.y}%.1f] @ $angle%.3f rad x $steps = [${exp.x}%.1f, ${exp.y}%.1f]/${expDist}%.2f"
            ) {
              ...
              dut.io.in.ready
                .expect(
                  true.B,
                  "DUT should be ready to accept new calculations"
                )
              ...
              val dist = dut.io.out.bits.dist.peek().toDouble
              ...
              got.norm should be(exp.norm +- 0.1)
              math.sqrt(dist) should be(expDist +- 0.1)
            }
          }
        }
      }
    ...
    it("determines if wall that was hit, was horizontal or vertical") {
      ...
      simulate(new Raycaster(12)) { dut =>
        tests.foreach { case (pos, angle) =>
          withClue(
            ...
            dut.clock.stepUntil(dut.io.out.valid, 1, 100)
            dut.io.out.valid.expect(true.B, "Calculation should be done")

            val (exp, dist) = RaycasterGoldenModel.expectedDdaPos(pos, angle, 0)
            dut.io.out.bits.pos.x.peek().toDouble should be(exp.x +- 0.1)
            dut.io.out.bits.pos.y.peek().toDouble should be(exp.y +- 0.1)

            dut.io.out.ready.poke(true.B)
            dut.io.stop.poke(true.B)
            dut.clock.step()

            dut.io.in.ready.expect(true.B, "Stop should have worked")
